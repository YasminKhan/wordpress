...
function book_upgrade_option() {
   add_theme_page( __( 'Upgrade', 'book' ), __( 'Upgrade', 'book' ), 'manage_options', 'book-upgrade', 'book_upgrade_content' );
}
...
function book_upgrade_content() {
	...
	<div class="wrap">
		<?php screen_icon(); ?>
		<h2><?php _e( 'Upgrade to Premium version&ndash;Book', 'book' ); ?></h2>
		<?php settings_errors(); ?>

		
		<?php echo __( '<h3>Missing Theme Options?</h3><p>Only Premium version of this theme&ndash;Book has Theme Options. You can upload a logo, select from over 300 Google Fonts, select different color themes and also write your own custom CSS if you need.</p>', 'book' ) ?>
		<p><img alt="Book Options" src="<?php echo get_template_directory_uri(); ?>/images/theme-options.png" /></p>
		<p><strong><a class="button-primary" href="http://www.wpshoppe.com/shop/book/"><?php echo __( 'Upgrade to Book Now!', 'book' ) ?></a></strong></p>
	</div>
	<?php
}
== WPHeadR ==
Designed for maximum content exposure in the header (HeadR) region plus the usual Bootstrap goodies.

Coded by Zulfikar Nore
Brought to you by WP Strap Code, http://wpstrapcode.com/

Requires at least:	3.4.1
Tested up to:		3.8-alpha
Stable tag:			1.0.1

== Copyright ==
WPHeadR, Copyright 2013 WPStrapCode.com
WPHeadR is distributed under the terms of the GNU GPL

WPHeadR is built with the following resources:
 
WPHeadR is built with the following resources: 
Underscores (_s) starter theme http://underscores.me/
Copyright: Automattic, automattic.com
Licensed under GPLv2 or later

Icon Fonts: Genericons - http://genericons.com/ 
Copyright: Automattic, automattic.com
Licensed under GPLv2 or later

Bootstrap - Copyright 2012 Twitter, Inc.
Licensed under the GPL-compatible license
http://www.apache.org/licenses/LICENSE-2.0 Apache License v2.0

Default banner image sourced from Pixabay.
Image url: http://pixabay.com/en/globe-e-mail-ball-earth-world-at-107394/
License: Public Domain CC0
License url: http://creativecommons.org/publicdomain/zero/1.0/deed.en

== Upgrade Notice ==

To be added if and when the need arises

== Frequently Asked Questions ==

* None Yet